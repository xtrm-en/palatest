...
public class GlyphAtlas {
    ...
    private final Map<Character, Glyph> glyphCharacterMap = new HashMap<>();

    private BufferedImage bufferedImage;
    private DynamicTexture loadedTexture;

    public GlyphAtlas(Font font, boolean antiAliasing, boolean fractionalMetrics) {
        this.font = font;
        this.antiAliasing = antiAliasing;
        this.fractionalMetrics = fractionalMetrics;
    }

    public void generateGlyphAtlas(char[] chars) {
        double maxWidth = -1;
        double maxHeight = -1;
    ...
    public void bindTexture() {
        glBindTexture(GL_TEXTURE_2D, loadedTexture.getGlTextureId());
    }
    ...
    public float drawChar(char ch, double x, double y) {
        ...
        float atlasX = glyph.x / (float) imgSize;
        float atlasY = glyph.y / (float) imgSize;

        float atlasWidth = glyph.width / (float) imgSize;
        float atlasHeight = glyph.height / (float) imgSize;
        ...
        glTexCoord2f(atlasX + atlasWidth, atlasY);
        glVertex2d(x + width, y);

        glTexCoord2f(atlasX, atlasY);
        glVertex2d(x, y);

        glTexCoord2f(atlasX, atlasY + atlasHeight);
        glVertex2d(x, y + height);

        glTexCoord2f(atlasX, atlasY + atlasHeight);
        glVertex2d(x, y + height);

        glTexCoord2f(atlasX + atlasWidth, atlasY + atlasHeight);
